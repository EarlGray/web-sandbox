...
subscribe fpath = do
  evch <- atomically $ STM.newTChan
  updch <- atomically $ STM.newTChan
  forkIO $ fswatcher evch fpath
  forkIO $ updwatcher evch updch
  return updch
...
fswatcher :: TChan FS.Event -> FilePath -> IO ()
fswatcher ch fpath = FS.withManager $ \fswatch -> do
...
updwatcher :: TChan FS.Event -> TChan FileUpdate -> IO ()
updwatcher inp out = go inp out M.empty
  where
    go inp out state = do
      ev <- chanRecv inp
